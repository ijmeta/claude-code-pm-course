...
Here's the scenario: You're the PM for Growth/Activation at FinTrack. Your bank connection rate (the percentage of new signups who successfully connect at least one bank account) has been stuck at 45% for the past 6 months. Leadership is frustrated and asking what you're going to do about it.
...
Ready to figure out what's wrong with FinTrack's activation?"
...
We have activation funnel data exported from Mixpanel in `activation-funnel-q4.csv`. This shows the key steps users go through: Signup → Bank Account Connected → First Budget Created → Budget Goal Set.
...
| Bank Account Connected | 10,000 | 7,200 | 72% | 18 min |
| First Budget Created | 7,200 | 2,880 | 40% | 45 min |
| Budget Goal Set | 2,880 | 1,440 | 50% | 24 min |

Found it! **60% of users who connect a bank account never create their first budget.** This is our problem.

Almost 3 out of 4 users connect a bank account, but then 6 out of 10 abandon before setting up their first budget. That's where we're losing people."

**STOP: Why do you think users are abandoning budgets after connecting their bank account?**
...
We have user survey responses in `user-survey-responses.csv` - 800 responses from recent signups. They answered questions about what confused them during onboarding and what they wish FinTrack had.
...
1. **'Didn't know how to set up budget categories'** - 35% of responses (280 users)
2. **'Overwhelmed by number of spending categories'** - 28% of responses (224 users)
3. **'Wasn't sure what budget amounts to set'** - 22% of responses (176 users)
4. **'Transactions were miscategorized and I didn't know where to start'** - 15% of responses (120 users)

**Sample quotes:**
- *'I connected my bank and then stared at hundreds of transactions not knowing what to do next'*
- *'Wish there were suggested budget amounts based on my spending so I had a starting point'*
- *'Coming from Mint, I expected it to just set up my budget automatically'*

**Segmentation insight:**
First-time budgeters mentioned this 2x more often than users switching from YNAB or Mint. They don't have established budget frameworks yet - they're figuring it out as they go.

**The data confirms it:** Users feel overwhelmed after connecting their bank account and need a guided setup to create their first budget. [If their theory was close: This aligns with what you said!] [If not: Interesting - data pointed us in a different direction!]"

**STOP: Based on this data, what feature would you propose to solve this problem?**

**CHECK:** Wait for student response (they might suggest: guided budget setup, suggested amounts, auto-categorization, budget templates, etc.)
...
For this module, we're going to move forward with **Guided Budget Setup with AI-suggested amounts**.

The concept: When new users connect their bank account, instead of showing them a blank budget builder, we'll launch a guided setup flow that analyzes their first 30 days of imported transactions and suggests budget category amounts based on their actual spending history. Users can accept, adjust, or skip each suggestion.

Users get a personalized starting budget in under 2 minutes, rather than staring at blank fields.
...
- Problem statement (60% drop-off between bank connection and first budget creation)
- Quantitative evidence (funnel data showing the drop-off)
- Qualitative evidence (survey quotes and top themes)
- Segmentation insight (especially bad for first-time budgeters - a core acquisition target post-Mint shutdown!)
- Proposed solution (Guided Budget Setup with AI-suggested amounts based on actual transaction history)
- Expected outcome (reduce drop-off by giving users a personalized starting point instead of a blank budget)
...
- Why it's happening (survey evidence: users overwhelmed after bank sync, no starting point for budgets)
- Who it affects most (first-time budgeters - a key segment of our post-Mint acquisition push)
- Proposed solution (Guided Budget Setup with AI-suggested amounts)
...
I can analyze your usage data in `fintrack-usage-data-q4.csv` and build the impact model for you. I'll process the event data, calculate current rates, segment by user type, and build out the full ROI projection."

**ACTION:**

Read `fintrack-usage-data-q4.csv` and analyze:
- Segment users by user_type (first-time budgeter vs. switcher from Mint/YNAB) and calculate activation rates
- Calculate current time-to-value (median time from signup to first budget created)
- Count total signups per month
- Calculate conversion rates to paying subscribers
...
- Projected state (assume 70% adoption of guided budget setup based on gradual rollout)
- Lift estimate: 45% → 58% activation (reasoning: 60% drop due to overwhelming blank budget setup, guided flow recovers ~30% of that drop-off conservatively)
- Business impact calculation:
  - Users affected: 5,000 signups/month × 70% see guided setup = 3,500/month
  - Current activated: 3,500 × 45% = 1,575/month
  - Projected activated: 3,500 × 58% = 2,030/month
  - Incremental: +455/month
  - Revenue: 455 × $9.99/mo × 60% conversion = $2,727 MRR = $32.7k ARR
  - 3-year LTV value: 455 × 19 months (avg lifetime) × $9.99 = $86,424
